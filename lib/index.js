...
class FFClient {
   ...
   */
  createRenderer({ width, height, backgroundImage } = {}) {
    return new FFRenderer(this, { width, height, backgroundImage });
  }
...
class FFRenderer {
   ...
   * @param {{ text: string, x: number, y: number, font?: string, size: string | number, color?: string, align?: "start" | "center" | "end" | { x: "start" | "center" | "end", y: "start" | "center" | "end" } | "start" | "center" | "end", shadow?: { color: string, x: number, y: number }, border?: { color: string, x: number, y: number }, type?: "native" | "canvas", weight?: "normal" | "bold" | number }} param0 
   */
   ...
   * @returns {Promise<{ clear: () => Promise<any>, result: { path: string, buffer: () => Promise<Buffer>, write: (path: string) => Promise<void> }, took: number }>}
   */
      ...
      mainFF.once("end", () => {
        ...
        resolve({
          ...
          result: {
            ...
            async write(p) {
              return await fs.promises.copyFile(outputFileName, p);
            }
          },
